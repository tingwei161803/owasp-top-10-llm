      ...
      article: function (p) {
        ...
        var body = (p.sections || []).map(function (s) {
          var blocks = (s.blocks || []).map(function (b) {
            ...
            if (b.type === "links") {
              var refs = b.items || [];
              return '<ul class="ref-links">' + refs.map(function (lk) {
                var label = esc(t(lk.label));
                var url = lk.url || "";
                if (!url) return '<li class="ref-links__item">' + label + "</li>";
                return '<li class="ref-links__item"><a href="' + esc(url) + '" target="_blank" rel="noopener noreferrer">' +
                  '<span class="ref-links__label">' + label + "</span>" +
                  '<span class="material-symbols-rounded ref-links__ext" aria-hidden="true">open_in_new</span></a></li>';
              }).join("") + "</ul>";
            }
            if (b.type === "ul") {
